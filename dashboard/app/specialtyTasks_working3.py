...
def main():
    ...
    2. Count data in a specific fiscal year
    ''')
    answer = input('> ')
...
def doubleCheckNWCSC(folder):
    ...
    Ok, let's try this again.''')
        ...
        print('''
    Sorry, I didn't understand that. Try typing 'Yes' or 'No'
     or the first letter of your answer.''')
        doubleCheckNWCSC(folder)

def FYdataCount(r, folder):
    """This may just create all the variables I will need, as well as create the dictionary of the
    ...
    levelOne(r, DataHostedElsewhere_id, PossiblePermissionsIssues_id,
             CouldNotFindFiles_id, totalProjects, projectDictNumber,
             projects, totalFYData, projectFiles, projectChildDictNumber,
             filesExist)

def levelOne(r, DataHostedElsewhere_id, PossiblePermissionsIssues_id,
             CouldNotFindFiles_id, totalProjects, projectDictNumber,
             projects, totalFYData, projectFiles, projectChildDictNumber,
             filesExist):
    """This function should go through every project in the fiscal year, and get it's children
    currentProject = sb.get_item(projects[projectDictNumber]) #This creates a variable for the
    current project folder in the FY folder """

    totalProjectFiles = None
    L7MissingData2 = False  #this automatically sets the default to "no missing data"
    isException = False
            #try:
    currentProject = sb.get_item(projects[projectDictNumber])
            #This creates a variable for the current project folder in the FY folder
            #isException = False
    projectChildDictNumber = 0
    print('''I am looking through the \''''+str(currentProject['title'])+''' project folder.''')
    L1Project.append(currentProject['title'])
    #This should add the project's title to a list for exporting to Excel later
    projectFiles = sb.get_child_ids(projects[projectDictNumber])
    #this gets the children of the project folder
    totalProjectFiles = None
    for i in projectFiles:
        if totalProjectFiles == None:
            ...
            totalProjectFiles += 1
            print("""The total number of items in this project is """+str(totalProjectFiles+1)
                  +""".""")
            #pprint(projectFiles) #Quantico
    levelTwo(r, DataHostedElsewhere_id, PossiblePermissionsIssues_id,
             CouldNotFindFiles_id, totalProjects, projectDictNumber,
             projects, totalFYData, projectFiles, projectChildDictNumber,
             filesExist, totalProjectFiles, isException, L7MissingData2)

            #except Exception:
            #    exception = projects[projectDictNumber]
            #    isException = True
            #    PossiblePermissionsIssues_id.append(exception)
            #    print('-------------------------------------WARNING(5): '+str(exception)+' raised
            #       an Exception or has a Permission issue.')
            #    print('-------------------------------------WARNING: Note that the above
            #       Exception or Permission issue is a project folder.')
            #    projectItemNextStep(r, DataHostedElsewhere_id, PossiblePermissionsIssues_id,
            #        CouldNotFindFiles_id, totalProjects, projectDictNumber, projects, totalFYData,
            #        projectFiles, projectChildDictNumber, filesExist, totalProjectFiles,
            #        isException, L7MissingData2)





def levelTwo(r, DataHostedElsewhere_id, PossiblePermissionsIssues_id, CouldNotFindFiles_id, totalProjects, projectDictNumber, projects, totalFYData, projectFiles, projectChildDictNumber, filesExist, totalProjectFiles, isException, L7MissingData2):
...
def issueDiagnostics(r, DataHostedElsewhere_id, PossiblePermissionsIssues_id, CouldNotFindFiles_id, totalFYData):
    ...
    NOTE: There appear to be items within the 'Approved DataSets' folder that
        ...
        print("Something went horribly wrong. Current function: doneCountingFY")
