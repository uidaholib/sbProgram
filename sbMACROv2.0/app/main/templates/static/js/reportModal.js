...
function createModal(sbId, projectArray) {
    ...
    for (let i = 0; i < project.pi_list.length; i++){
        ...
        if (project.pi_list[i].email) {
            currPiHtml = '<a href="mailto:' + project.pi_list[i].email + '">' + currPiHtml + '</a>';
        }
        if (i === 0) {
    ...
    for(let i = 0; i < prodRecArray.length; i++)
    {
        if (prodRecArray[i].name){
            ...
            html += '<a href="' + prodRecArray[i].url + '" target="_blank">'
                + (i+1) + '. '
                + prodRecArray[i].name + '</a><br>'
        }
    }
    if(html != ""){
    ...
    } else if (project.dmp_status === "None" || project.dmp_status === "none" || project.dmp_status === "n/a") {
        $("#" + modal_id).find(".DMPstatus_neutral").css("display", "inline-block");
    } else if (project.dmp_status === "Project not currently tracked by Data Steward") {
        $("#" + modal_id).find(".DMPstatus_neutral").css("display", "inline-block");
    } else {
