...
function createPieChart(d3, project, modalId) {
    ...
    let svg = d3.select('#'+modalId+' #modal_chart')
        .append('svg')
        .attr('width', chartSize.width)
        .attr('height', chartSize.height)
        .append('g')
        .attr('transform', 'translate(' + (chartSize.width / 2) +
            ',' + (chartSize.height / 2) + ')');
    ...
    let tooltip = d3.select('#' + modalId + ' #modal_chart')
        .append('div')
        .attr('class', 'tooltip');
    tooltip.append('div')
        .attr('class', 'label');
    ...
    path.on('mouseover', function (d) {
        ...
        tooltip.select('.percent').html("<strong>Percentage of Total: </strong>" + percent + '%');
